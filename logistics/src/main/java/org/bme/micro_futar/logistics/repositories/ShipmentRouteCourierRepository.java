...
public interface ShipmentRouteCourierRepository extends JpaRepository<ShipmentRouteCourier, Long> {
    ...
    long countByCourierIdAndDate(@Param("courierId") Long courierId, @Param("date") Date date);

    List<ShipmentRouteCourier> findByCourierIdAndDateAssignedFor(Long courierId, Date date);
}
